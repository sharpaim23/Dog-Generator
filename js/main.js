...
fetch(BREEDS_URL)
  ...
  .then(data => {
    const breedsObject = data.message; //Turns the message into an object
    const breedsArray = Object.keys(breedsObject) //Turns the object into an array
      for(let i = 0; i < breedsArray.length; i++ ){
        ...
        select.appendChild(option) // add current <option> tag to the select box list of options
      }
  })
